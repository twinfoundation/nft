...
import type { IIotaStardustNftConnectorConfig } from "./models/IIotaStardustNftConnectorConfig";
import type { IIotaStardustNftConnectorConstructorOptions } from "./models/IIotaStardustNftConnectorConstructorOptions";

/**
 * Class for performing NFT operations on IOTA Stardust.
 */
export class IotaStardustNftConnector implements INftConnector {
	/**
	 ...
	 */
	public readonly CLASS_NAME: string = nameof<IotaStardustNftConnector>();
	 ...
	 * @internal
	 */
	private readonly _config: IIotaStardustNftConnectorConfig;

	/**
	 * Create a new instance of IotaNftConnector.
	 * @param options The options for the connector.
	 */
	constructor(options: IIotaStardustNftConnectorConstructorOptions) {
		Guards.object(this.CLASS_NAME, nameof(options), options);
		Guards.object<IIotaStardustNftConnectorConfig>(
			this.CLASS_NAME,
			nameof(options.config),
			options.config
		);
		Guards.object<IIotaStardustNftConnectorConfig["clientOptions"]>(
			this.CLASS_NAME,
	...
	public async mint<T = unknown, U = unknown>(
		...
		try {
			...
			return `nft:${new Urn(IotaStardustNftConnector.NAMESPACE, `${hrp}:${nftId}`).toString()}`;
		} catch (error) {
	...
	public async resolve<T = unknown, U = unknown>(
		...
		if (urnParsed.namespaceMethod() !== IotaStardustNftConnector.NAMESPACE) {
			throw new GeneralError(this.CLASS_NAME, "namespaceMismatch", {
				namespace: IotaStardustNftConnector.NAMESPACE,
				id
			});
		}
